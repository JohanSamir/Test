...
def create_runner(base_dir, schedule='continuous_train_and_eval'):
  ...
  if schedule == 'continuous_train_and_eval':
    return Runner_new(base_dir, create_agent)
  # Continuously runs training until max num_iterations is hit.
  elif schedule == 'continuous_train':
    return TrainRunner_new(base_dir, create_agent)
  else:
    raise ValueError('Unknown schedule: {}'.format(schedule))


@gin.configurable
class Runner_new(object):
  ...
  def _initialize_episode(self, iteration):
    ...
    return self._agent.begin_episode(initial_observation, iteration)
  ...
  def _run_one_episode(self, iteration):
    ...
    action = self._initialize_episode(iteration)
    is_terminal = False

    # Keep interacting until we reach a terminal state.
    while True:
      observation, reward, is_terminal = self._run_one_step(action)
      #print('while true observation:', observation, 'reward:', reward, 'is_terminal:', is_terminal)

      total_reward += reward
      step_number += 1

      if self._clip_rewards: #False
  ...
  def _run_one_phase(self, min_steps, statistics, iteration, run_mode_str):
    ...
    self._agent.init_eps() # TODO: I am not using this metod to explore and exploit
  ...
  def _log_experiment(self, iteration, statistics):
    ...
    #print('logging_file_prefix:',iteration, 'statistics:',statistics)
...
@gin.configurable
class TrainRunner_new(Runner_new):
  ...
  def __init__(self, base_dir, create_agent_fn,
    ...
    super(TrainRunner_new, self).__init__(base_dir, create_agent_fn,
  ...
  def _run_one_iteration(self, iteration):
    ...
    statistics = iteration_statistics.IterationStatistics()
    num_episodes_train, average_reward_train, average_steps_per_second = (
        self._run_train_phase(statistics, iteration))
    print('num_episodes_train:',num_episodes_train,'average_reward_train:', average_reward_train)
    self._save_tensorboard_summaries(iteration, num_episodes_train,
                                     average_reward_train,
                                     average_steps_per_second)
    return statistics.data_lists
